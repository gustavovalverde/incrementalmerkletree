            ...
            if to_clear.is_empty() {
                ...
                match &root.0 {
                    ...
                    Node::Leaf { value: (h, r) } => {
                        ...
                        // A single-position branch descends to the level-0 leaf it targets. A
                        // folded leaf (a subtree pruned to one annotated node) can instead span
                        // several `to_clear` positions at once: `unite` only folds when the left
                        // child is ephemeral and the right is neither marked nor a reference, so
                        // a folded leaf carries at most the rightmost position's checkpoint flag
                        // and never a mark. Every other position in range was already pruned, so
                        // clearing the union of the requested flags drops the live flag and is a
                        // no-op for the flags that folding already removed.
...
mod tests {
    ...
    fn clear_flags_across_a_folded_leaf() {
        ...
        let tree =
            LocatedTree::from_parts(root_addr, parent(folded, nil())).unwrap();
